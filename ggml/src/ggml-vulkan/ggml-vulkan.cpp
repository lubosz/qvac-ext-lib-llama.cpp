...
static vk_buffer ggml_vk_create_buffer(vk_device& device, const vk::BufferCreateInfo& buffer_info, const VmaAllocationCreateInfo& alloc_info) {
    ...
    vk::BufferDeviceAddressInfo address_info(buf->buffer);
    buf->address = device->device.getBufferAddress(address_info);
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        if (vk12_features.storageBuffer8BitAccess == VK_FALSE) {
...
static void ggml_vk_mul_mat_vec_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool dryrun = false) {
    ...
    if (dmmv->name == "mul_mat_vec_q4_0_q8_1_f32") {
        const vk_mat_vec_push_constants_addr pc = {
                ...
                static_cast<uint64_t>(d_X->address + x_buf_offset),
                static_cast<uint64_t>(d_Y->address + y_buf_offset),
                static_cast<uint64_t>(d_D->address + d_buf_offset)
        };
